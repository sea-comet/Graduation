...
def visualize_data_gif(data_):
    ...
    :param data_: input labeled image  （240， 240， 150， 3）
    :return:  返回每个面得动态分割图
    '''
    images = []
...
def visualize_patch(X, y):
    ...
    :param X: X (np.array): sample of original image of dimension
            (num_channels, output_x, output_y, output_z)
            输入进来的是4维，但是只能传3维进去才能显示，>>>>  X[0, :, :, :]
    :param y:  (np.array): labels which correspond to X, of dimension
            (num_classes, output_x, output_y, output_z)   num_classes: 不算背景
            同X, 只能查看某一类的标签，不能查看所有类的， 查看第二类的标签 >> y[1, :, :, :]
    :return:
    '''
    fig, ax = plt.subplots(1, 2, figsize=[10, 5], squeeze=False)

    ax[0][0].imshow(X[:, :, 0], cmap='Greys_r')   #  只显示一个二维平面
    ax[0][0].set_yticks([])
    ax[0][0].set_xticks([])
...
def get_labeled_image(image, label, is_categorical=False):
    ...
    :param is_categorical:  if false, label to one-hot
    :return:  返回吧label嵌入image的图像
    '''
    if not is_categorical:
        label = to_categorical(label, num_classes=4).astype(np.uint8)  # (240, 240, 155, 4)

    image = cv2.normalize(image[:, :, :, 0], None, alpha=0, beta=255,
                          norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F).astype(np.uint8)
    # 归一化， 范围[0, 255]

    labeled_image = np.zeros_like(label[:, :, :, 1:])  # (240, 240, 155, 3)
    ...
    return labeled_image  # (240, 240, 155, 3)
...
def get_sub_volume(image, label,
    ...
    Args:
        ...
        取样时，由于需要取到5%的肿瘤区域，如果随机没有取到，则重新取样，该参数表示最多重取几次样
...
# 标准化数据， 均值为0， 方差为1. 注意：标准化是分别对每个二维平面进行标准化
