...
class Brats(Dataset):
    def __init__(self, patients_dir, benchmarking=False, training=True, debug=False, data_aug=False,
        ...
        self.datas = []
        self.validation = no_seg # no_seg 代表用于validation和testing，不需要seg.nii
        self.patterns = ["_t1", "_t1ce", "_t2", "_flair"]
        if not no_seg: # 说明用于training
            self.patterns += ["_seg"]
        for patient_dir in patients_dir:
            patient_id = patient_dir.name # 取出来病人代表的Number
            ...
            self.datas.append(patient)  # datas里面放了数据集里所有病人的各模态.nii和seg.nii数据的path,每个病人有个dict字典

    def __getitem__(self, idx):
        _patient = self.datas[idx]  # _patient 为某病人id和4模态以及seg的path字典, load_nii 函数在下面, 传参为.nii文件的地址
        # patient_image：某病人4个模态t1, t1ce, t2, flair 的array，组成的字典
        ...
        if _patient["seg"] is not None:
            ...
            patient_label = np.stack([et, tc, wt])  # 增维度，是3个分类class，数字为3
        else:
        ...
        if self.training: # 训练！！
            ...
            # Add 1 pixel in each side #　就在最小值和最大值上加了一个pixel !!切出最小能包括整个肿瘤的框！！
            ...
            # default to 128, 128, 128　　　　＃　pad_or_crop_image函数在dataset下面的image_utils.py 里面
            patient_image, patient_label = pad_or_crop_image(patient_image, patient_label, target_size=(128, 128, 128))
               ...
               # 专门的数据预处理 --> determinist_collate，不先crop
            z_indexes, y_indexes, x_indexes = np.nonzero(np.sum(patient_image, axis=0) != 0)
            # Add 1 pixel in each side
            zmin, ymin, xmin = [max(0, int(np.min(arr) - 1)) for arr in (z_indexes, y_indexes, x_indexes)]
            zmax, ymax, xmax = [int(np.max(arr) + 1) for arr in (z_indexes, y_indexes, x_indexes)]
            # 因为每个肿瘤的大小框都不一定一样，所以很多要pad
            patient_image = patient_image[:, zmin:zmax, ymin:ymax, xmin:xmax]
            patient_label = patient_label[:, zmin:zmax, ymin:ymax, xmin:xmax]
...
def get_datasets(seed, debug, no_seg=False, on="train", full=False,
    ...
    if full: # 如果用了整个数据集来training and validation,就是full,没有用 5 fold 交叉验证
    ...
    kfold = KFold(5, shuffle=True, random_state=seed)  # 5折验证，用了seed, 确保每次一样
    ...
    # return patients_dir # 用了 5 fold 交叉验证的, 用了五分之四的数据来train,五分之一来validate
